...
FIELD_ALIASES = {
    "Participant ID": ["ParticipantID", "Respondent ID", "User ID", "participant_id"],
    "Already Flagged?": ["Already Flagged", "Flagged", "Is Flagged"],
    "Original Signup ID": ["OriginalSignupID", "Signup ID", "SignupID", "invitation_id"],
    "Create Date": ["Created Date", "Created At", "Signup Date", "Date", "create_date"],
    ...
    "Age Group": ["Age"],
    "Ethnicity": [],
    "Affiliation": ["Organization", "Organisation", "Employer", "affiliation_a"],
    "Phone": ["Phone Number", "Mobile", "Cell", "Cell Phone", "Telephone"],
    "Contact Preference": ["ContactPref", "Preferred Contact", "contact_preference"],
    "UTM Medium": ["UTM_Medium", "Utm Medium", "utm_medium"],
    "Total Referrals": ["Referrals Total", "Referral Count"],
    "Referral Feedback": [],
    "Feedback Group": ["feedback_group"],
    "Gross Feedback": ["gross_feedback"],
    "Company Suggestions": ["Company Suggestion", "company_suggestions"],
    "HXC Program Feedback": ["HXC Feedback", "Program Feedback", "hxc_program_feedback"],
    "Over Program Limit (>75)": [
        "Over Program Limit",
        "Over Program Limit (> 75)",
        "Over Program Limit (75+)",
    ...
    "Elig. Feedback": ["Eligibility Feedback", "Elig Feedback", "eligible_feedback"],
    "Feedback Left to Give": [],
    "Payout Elig. Feedback (Last 300 Days)": [
    ...
    "Affiliation 2": ["Affiliation2", "Organization 2", "Program", "affiliation_b"],
}
...
def load_data_from_source(df: pd.DataFrame) -> None:
    df = df.reset_index(drop=True)
    df_working, mapping = standardize_dataframe(df)
...
def render_step1() -> bool:
    """Render the data source selection. Returns True when data is ready."""
    st.header("Step 1: Load Participant List")

    source = st.radio(
        "Select data source:",
        ["Upload CSV / XLSX", "Connect to BigQuery"],
        horizontal=True,
    ...
    if source == "Upload CSV / XLSX":
        uploaded_file = st.file_uploader(
            "Upload participant file", type=["csv", "xlsx"], label_visibility="collapsed"
        )
        ...
        if st.session_state["upload_token"] != upload_token:
            ...
            load_data_from_source(df_raw)
            st.session_state["upload_token"] = upload_token

        st.success(f"File loaded — {len(st.session_state['df_original']):,} rows.")
        return True
    ...
    if not required_filled:
        st.info("Fill in Sign Up Solution ID, Program Solution ID, and Feedback Limit to run the query.")

    if st.button("Run Query", type="primary", disabled=not required_filled):
        with st.spinner("Running BigQuery query…"):
            try:
                bq_df = run_participant_query(
                ...
                if bq_df.empty:
                    st.error("Query returned no rows. Check your parameters.")
                    return False

                load_data_from_source(bq_df)
                st.session_state["upload_token"] = (
                    f"bq:{sign_up_solution_id}:{program_solution_id}:{feedback_limit}"
                )
                st.success(f"Query returned {len(bq_df):,} rows.")
            except Exception as e:
                st.error(f"Query failed: {e}")
                return False

    return st.session_state.get("df_original") is not None
...
def main() -> None:
    ...
    st.caption("Load participant data, find likely duplicate sign-ups, review groups, and export decisions.")
    ...
    st.header("Step 2: Find Duplicate Candidates")
    ...
    program_ppf = st.number_input(
        ...
        step=0.1,
        format="%.2f",
        help="Program-level PPF value used in Total Value calculation. Default is 1.6.",
    )
    ...
    st.divider()
    st.header("Step 3: Review Potential Matches")
    ...
    st.divider()
    st.header("Step 4: Build Outputs")
    st.write(
    ...
    if not REPORTLAB_AVAILABLE:
        ...
        if logo_path is None:
            st.caption(
                "Logo file not found. Set HUNDREDX_LOGO_PATH or place the logo at "
